...
public class BanksBankStanderScript extends Script {
    ...
    private boolean combineItems() {
        ...
        if (firstItemId != null && secondItemId != null) {
            Rs2Inventory.interact(items().stream().filter(x -> x.id == firstItemId).findFirst().orElse(null), menu); // Use first Rs2Item (random or not)

            if (config.secondItemQuantity() > 0) {
                Rs2Inventory.interact(items().stream().filter(x -> x.id == secondItemId).findFirst().orElse(null), menu);
            }
            ...
            if (config.secondItemQuantity() > 0) {
                Rs2Inventory.interact(items().stream().filter(x -> x.name.equalsIgnoreCase(secondItemIdentifier.toLowerCase())).findFirst().orElse(null), menu);
            }
        } else if (secondItemId != null) {
